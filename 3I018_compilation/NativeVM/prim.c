...
void do_eqeq_prim(varray_t *stack) {
  int r = 0; // par défaut le résultat est faux
  value_t *stack0 = varray_top_at(stack,0);
  value_t *stack1 = varray_top_at(stack,1);
  
  // Tester si les types des arguments sont égaux
  if((stack0->type == T_INT || stack0->type == T_BOOL)
     && (stack1->type == T_INT || stack1->type == T_BOOL)) {
    r = stack0->data.as_int == stack1->data.as_int;
    ...
    printf("Unable to apply eq with type %d and type %d\n",
	   stack0->type, stack1->type);
    abort();
  }
}

void do_eqeqeq_prim(varray_t *stack) {
  int r = 0; // par défaut le résultat est faux
  value_t *stack0 = varray_top_at(stack,0);
  value_t *stack1 = varray_top_at(stack,1);
  ...
  if ((stack0->type == T_INT || stack0->type == T_BOOL)
      && (stack1->type == T_INT || stack1->type == T_BOOL)) {
    r = (stack0->type == stack1->type) &&
           stack0->data.as_int == stack1->data.as_int;
...
/********** Correction - TD11 **********/
    ...
    // égalité
  case P_EQ:
    do_eqeq_prim(stack); break;
    break;
  case P_EQ_WITH_TYPE :
    do_eqeqeq_prim(stack); break;
    break;
